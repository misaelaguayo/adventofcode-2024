...
fn equation_solver(ans: i64, current: i64, equation: &Vec<i64>) -> bool {
    ...
    let concat = format!("{}{}", current.to_string(), equation[0].to_string()).parse::<i64>().unwrap();

    let new_eq = equation[1..].to_vec();
    equation_solver(ans, mul, &new_eq) || equation_solver(ans, add, &new_eq) || equation_solver(ans, concat, &new_eq)
}
